...
const fetch_all_requirements = function(){
    ...
    return new Promise(function(resolve , reject){
      ...
      var sql_query = `SELECT * FROM requirement WHERE fullfilled = 0 and category = 0 ORDER BY emergency_value DESC;`
...
const fetch_posted = function(id){
  ...
  return new Promise(function(resolve , reject){
    ...
    FROM requirement r
    WHERE r.orphanage_id = '${id}' and r.fullfilled = 0 and r.category = 0
...
const fetch_donations = function(id){
  ...
  return new Promise(function(resolve , reject){
    ...
    JOIN requirement ON donations.requirement_id = requirement.requirement_id and requirement.category = 0
...
router.post('/donate', async (req, res) => {
  ...
  const donatedQuantity = parseFloat(data.donated_quantity);
  const requirementId = parseInt(data.requirement_id);

  if (isNaN(donatedQuantity) || donatedQuantity <= 0 || isNaN(requirementId)) {
    return res.status(400).send('Invalid donation data. Please provide valid values.');
  }

  const fullfilled = data.old_quantity - donatedQuantity <= 0 ? 1 : 0;
  ...
  try {
    ...
    await con.query(
      'UPDATE requirement SET quantity = quantity - ?, fullfilled = ? WHERE requirement_id = ?;',
      [donatedQuantity, fullfilled, requirementId]
    );

    // Use parameterized query to prevent SQL injection
    await con.query(
      'INSERT INTO donations (doner_id, requirement_id, dquantity) VALUES (?, ?, ?);',
      [userId, requirementId, donatedQuantity]
    );
...
const fetch_posted_volunteer = function(id){
  ...
  return new Promise(function(resolve , reject){
    ...
    var sql_query = ` SELECT r.*
    FROM requirement r
    WHERE r.orphanage_id = '${id}' and r.fullfilled = 0 and r.category = 1
    ORDER BY requirement_id DESC;`
    con.query(sql_query, function (err, result, fields) {
      if (err) {
          return reject(err);
